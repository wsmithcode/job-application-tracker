...
export class JobResearchController {
  ...
  @Get()
  async getJobResearches(@CurrentUser() user: User) {
  ...
  async createJobResearch(
    ...
    const userId = user.id;
    const jobResearch = await this.jobResearch.createJobResearch(
      userId,
  ...
  async updateJobResearch(
    ...
    const jobResearch = await this.jobResearch.updateJobResearch(
      id,
  ...
  @Delete(':id')
  async deleteJobResearch(@Param('id') id: string) {
